...
function getLocalAgentToolSpecs() {
  const base = [...AGENT_TOOL_SPECS, ...ADVANCED_AGENT_TOOL_SPECS, ...LOCAL_ONLY_AGENT_TOOL_SPECS];
  if (typeof pluginManager !== 'undefined' && pluginManager) {
    const pluginTools = pluginManager.getToolSpecs();
    for (const pt of pluginTools) {
      if (!base.some(t => t.name === pt.name)) base.push(pt);
    }
  }
  return base;
}
...
function buildAgentToolInstructions(toolSpecs = null, runtimeKind = 'local', agentType = 'general-purpose') {
  const specs = toolSpecs || getLocalAgentToolSpecs();
  const toolLines = specs
...
function getAgentSystemPrompt(options = {}) {
  ...
  const toolSpecs = runtimeKind === 'cloud' ? CLOUD_AGENT_TOOL_SPECS : getLocalAgentToolSpecs();
...
async function enforceAgentShellPolicy(command) {
  if (!agentAllowShell()) {
  ...
  if (mutationGuard && cfg('guard.enabled') !== false) {
    ...
    if (shellCheck.requiresApproval) {
      ...
      const approved = await mutationGuard.requestApproval('shell', command.slice(0, 200), agentType, shellCheck.reason);
      if (!approved) {
        throw new Error(`Shell command denied by user: ${shellCheck.reason}`);
      }
    }
  }
...
async function runLocalShellCommand(command, cwd, timeoutMs, options = {}) {
  await enforceAgentShellPolicy(command);
...
async function executeAgentToolCall(toolCall, execContext = {}) {
  ...
  switch (name) {
    ...
    default:
      if (typeof pluginManager !== 'undefined' && pluginManager) {
        const pluginResult = await pluginManager.executeTool(name, input, {
          agentId: currentTask ? currentTask.agentId : '',
          agentType,
          taskId: execContext.taskId || '',
          rootPath
        });
        if (pluginResult.ok) return pluginResult.result;
        throw new Error(pluginResult.error || `Plugin tool '${name}' failed`);
      }
    ...
    for (const toolCall of toolCalls) {
      ...
      if (toolCall.name === 'write_file' && toolCall.input && mutationGuard && cfg('guard.enabled') !== false) {
        ...
        if (!guardResult.allowed) {
          ...
          toolResultBlocks.push(buildToolResultBlockForModel(toolCall, `Write blocked by MutationGuard: ${guardResult.reason}`, false));
          executedTools.push({ name: toolCall.name, input: toolCall.input, ok: false, error: guardResult.reason });
          if (hooks.onToolResult) hooks.onToolResult(toolCall, guardResult.reason, false, round + 1);
          continue;
        }
        if (guardResult.requiresApproval) {
          ...
          const approved = await mutationGuard.requestApproval('write', writePath, agentType, guardResult.reason);
          if (!approved) {
            toolResultBlocks.push(buildToolResultBlockForModel(toolCall, `Write to "${writePath}" denied by user.`, false));
            executedTools.push({ name: toolCall.name, input: toolCall.input, ok: false, error: 'User denied approval' });
            if (hooks.onToolResult) hooks.onToolResult(toolCall, 'User denied approval', false, round + 1);
            continue;
          }
        }
      }
    ...
    try {
      ...
      if (cfg('guard.enabled') !== false) {
        mutationGuard = new MutationGuard({
          onApprovalNeeded: async ({ operation, target, agentType, reason }) => {
            const choice = await vscode.window.showWarningMessage(
              `[MutationGuard] Agent "${agentType}" wants to ${operation}: ${target}\nReason: ${reason}`,
              { modal: true },
              'Allow', 'Deny'
            );
            return choice === 'Allow';
