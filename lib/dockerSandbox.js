...
function writeToolRunnerFile(workspaceDir) {
  ...
  // Write an ESM-safe shim at the legacy .js path so that even if old code or
  // a stale container calls tool-runner.js inside a "type":"module" project,
  // it delegates to the real .cjs runner instead of crashing with
  // "ReferenceError: require is not defined".
  const legacyRunnerPath = path.join(workspaceDir, LEGACY_INTERNAL_TOOL_RUNNER);
  const esmShim = [
    '// ESM shim — delegates to tool-runner.cjs for "type":"module" projects',
    'import { createRequire } from "module";',
    'const require = createRequire(import.meta.url);',
    'require("./tool-runner.cjs");'
  ].join('\n') + '\n';
  try {
    fs.writeFileSync(legacyRunnerPath, esmShim, 'utf8');
  } catch (_) {}
}
...
class DockerSandboxManager {
  ...
  async tryAutoStartDocker() {
    ...
    if (platform === 'win32') {
      console.log(`[Docker] tryAutoStartDocker: candidate search started (Win32)`);
      const candidates = [
      ...
      for (const candidate of candidates) {
        console.log(`[Docker] Checking candidate: ${candidate}`);
        try {
          if (fileExists(candidate)) {
            console.log(`[Docker] Candidate found! Launching: ${candidate}`);
            spawn(candidate, [], { detached: true, stdio: 'ignore' }).unref();
            launched = true;
            break;
          }
        } catch (e) {
          console.error(`[Docker] Error checking candidate ${candidate}:`, e);
        }
      }
      if (!launched) {
        console.warn(`[Docker] No Docker Desktop executable found among candidates.`);
        return false;
      }
    ...
    console.log(`[Docker] Launched. Waiting for daemon to be ready (timeout: 90s)...`);
    ...
    while (Date.now() - startedAt < maxWaitMs) {
      ...
      try {
        ...
        console.log(`[Docker] Still waiting... (${Math.round((Date.now() - startedAt) / 1000)}s)`);
      }
    }
    console.error(`[Docker] Auto-start timeout: daemon did not become ready in 90s.`);
