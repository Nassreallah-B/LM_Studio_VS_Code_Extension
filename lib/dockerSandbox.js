...
const INTERNAL_DIR_NAME = '.localai-sandbox';
const INTERNAL_TOOL_RUNNER = `${INTERNAL_DIR_NAME}/tool-runner.js`;
...
function copyWorkspaceTree(sourceRoot, targetRoot, relativePath = '') {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      copyWorkspaceTree(sourceRoot, targetRoot, childRelative);
      continue;
    }
...
class DockerSandboxManager {
  constructor(options = {}) {
    ...
    this.image = String(options.image || 'localai-code-sandbox:latest').trim();
    this.networkMode = String(options.networkMode || 'none').trim() || 'none';
    this.autoBuild = options.autoBuild !== false;
    this.containerNamePrefix = String(options.containerNamePrefix || 'localai-sbx').trim() || 'localai-sbx';
  ...
  async ensureReady() {
    ...
    if (!health.dockerReady) {
      throw new Error(`Docker sandbox runtime unavailable: ${health.detail || 'docker daemon is not reachable.'}`);
    }
    if (health.imageReady) return health;
    if (!this.autoBuild) {
  ...
  async createFromWorkspace(input) {
    ...
    copyWorkspaceTree(path.resolve(input.sourceRoot), workspaceDir);
  ...
  async _finalizeSandbox(meta) {
    ...
    ensureDirSync(path.join(meta.workspaceDir, INTERNAL_DIR_NAME));
    fs.writeFileSync(path.join(meta.workspaceDir, INTERNAL_TOOL_RUNNER), getToolRunnerSource(), 'utf8');
  ...
  async attach(metaOrId) {
    ...
    await runProcess('docker', [
      ...
      '/workspace',
      '--mount',
      `type=bind,src=${meta.workspaceDir},dst=/workspace`,
      '--network',
      this.networkMode,
      this.image,
      'sh',
  ...
  async _execContainer(meta, args, options = {}) {
    return runProcess('docker', ['exec', meta.containerName, ...args], {
  ...
  async _checkGitAvailable(meta) {
    try {
      ...
      console.warn(`[localai-code] Git unavailable in sandbox ${meta.id}: ${error.message}`);
      return false;
    }
  }
  ...
  async _validateGitState(meta) {
    ...
    if (!await this._checkGitAvailable(meta)) {
      warnings.push('git unavailable inside sandbox container');
      return warnings;
    }
    try {
      ...
      warnings.push(`git state validation failed: ${error.message}`);
      meta.gitAvailable = false;
    }
    return warnings;
  }
  ...
  async _createFileBasedSnapshot(meta) {
    ...
    while (stack.length) {
      ...
      try {
        ...
        console.warn(`[localai-code] Failed to read sandbox snapshot directory ${absoluteDir}: ${error.message}`);
        continue;
      }
      for (const entry of entries) {
        const relativePath = normalizeRelativePath(relativeDir ? path.posix.join(relativeDir, entry.name) : entry.name);
        if (!relativePath || relativePath.startsWith(INTERNAL_DIR_PREFIX) || relativePath === INTERNAL_DIR_NAME) continue;
        ...
        try {
          ...
          baseline[relativePath] = createHash('sha256').update(buffer).digest('hex');
        } catch (error) {
          console.warn(`[localai-code] Failed to hash sandbox file ${relativePath}: ${error.message}`);
        }
      }
    }
    const snapshotDir = path.join(root, INTERNAL_DIR_NAME, 'file-snapshots');
    ...
    meta.fileSnapshotPath = normalizeRelativePath(path.relative(root, snapshotPath));
    meta.fileSnapshot = baseline;
    return baseline;
  }

  async _initializeBaseline(meta) {
    if (meta.baselineReady) return;
    fs.writeFileSync(path.join(meta.workspaceDir, INTERNAL_METADATA_FILE), JSON.stringify({
      sandboxId: meta.id,
      createdAt: meta.createdAt,
      image: meta.image
    }, null, 2), 'utf8');
    const warnings = await this._validateGitState(meta);
    if (meta.gitAvailable !== false) {
      try {
        await this._execContainerShell(meta, `cd /workspace && ${[
          'if [ ! -d .git ]; then git init -q; fi',
          `git config user.email '${escapeShellSingleQuotes('sandbox@localai-code.invalid')}'`,
          `git config user.name '${escapeShellSingleQuotes('LocalAI Sandbox')}'`,
          'git add -A',
          'git commit --allow-empty -qm "Sandbox baseline" || true'
        ...
        warnings.push(`git baseline failed: ${error.message}`);
        meta.gitAvailable = false;
      }
    }
    if (meta.gitAvailable === false || !meta.baseRef) {
      await this._createFileBasedSnapshot(meta);
      meta.baseRef = '';
      meta.lastCommittedRef = '';
    }
  ...
  async commitCheckpoint(metaOrId, message = 'sandbox checkpoint') {
    ...
    if (!meta) throw new Error('Sandbox metadata not found.');
    await this.attach(meta);
    if (meta.baselineMethod === 'file-snapshot' || meta.gitAvailable === false) {
      return '';
    }
    await this.execShell(meta, [
      'git add -A',
      `git commit --allow-empty -qm '${escapeShellSingleQuotes(message)}' || true`
    ].join(' && '), '.', { timeoutMs: 120000 });
  ...
  async restoreToRef(metaOrId, gitRef) {
    ...
    if (meta.baselineMethod === 'file-snapshot' || meta.gitAvailable === false) {
      console.warn(`[localai-code] Git restore skipped for sandbox ${meta.id}: git baseline unavailable.`);
      return meta;
    }
    await this.attach(meta);
    await this.execShell(meta, [
  ...
  async _collectFileBasedPatch(meta) {
    const baseline = meta.fileSnapshot || {};
    const current = {};
    const root = meta.workspaceDir;
    const stack = [''];
    while (stack.length) {
      ...
      try {
        ...
        console.warn(`[localai-code] Failed to read sandbox patch directory ${absoluteDir}: ${error.message}`);
        continue;
      }
      for (const entry of entries) {
        const relativePath = normalizeRelativePath(relativeDir ? path.posix.join(relativeDir, entry.name) : entry.name);
        ...
        try {
          ...
          console.warn(`[localai-code] Failed to collect sandbox patch file ${relativePath}: ${error.message}`);
        }
      }
    }
    ...
    for (const relativePath of allPaths) {
      const beforeHash = baseline[relativePath];
      const afterEntry = current[relativePath];
      if (!beforeHash && afterEntry) {
        files.push({ type: 'add', path: relativePath, beforeText: null, afterText: afterEntry.text });
        continue;
      }
      if (beforeHash && !afterEntry) {
        files.push({ type: 'delete', path: relativePath, beforeText: null, afterText: null });
        continue;
      }
      if (beforeHash && afterEntry && beforeHash !== afterEntry.hash) {
        files.push({ type: 'modify', path: relativePath, beforeText: null, afterText: afterEntry.text });
      }
    }
    ...
    return {
      ...
      diffText: '',
      files,
      summary: files.length ? `${files.length} file change(s) ready for host review` : 'No workspace changes were produced inside the sandbox.'
    };
  }
