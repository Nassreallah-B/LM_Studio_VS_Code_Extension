...
const AGENT_TYPE_CATALOG = {
  'aria-orchestrator': {
    ...
    instruction: 'You are ARIA, the Lead Orchestrator. Your role is to analyze complex needs, identify impact areas (UI, DB, Security), and delegate to specialized subagents. Do not code directly; coordinate, verify proofs of test, and provide the final GO for deployment. IMPORTANT: When spawning subagents, you MUST use `wait_agent` and `task_output` to collect and synthesize their results before providing your final answer. Report your phase at the start of your message using `[PHASE: PLANNING]`, `[PHASE: EXECUTION]`, or `[PHASE: VERIFICATION]`.'
  },
  'general-purpose': {
...
const ADVANCED_AGENT_TOOL_SPECS = [
  ...
  { name: 'web_fetch', description: 'Fetch a web page and return normalized text content. maxChars defaults to the configured value (4000).', example: '{"url":"https://example.com/docs","maxChars":4000}' },
...
class RuntimeFeatureStore {
  constructor(options) {
    ...
    this.configGetter = options.configGetter || null;
    this.memoryDB = options.memoryDB || null; // MemoryDB bridge for unified persistence
  ...
  saveOnboarding() {
    ...
    // ── MemoryDB dual-write: persist onboarding to structured store ───────
    if (this.memoryDB) {
      try {
        this.memoryDB.store('onboarding', 'summary', this.onboardingState.summary || '');
        this.memoryDB.store('onboarding', 'conventions', this.onboardingState.conventions || []);
        this.memoryDB.store('onboarding', 'riskyZones', this.onboardingState.riskyZones || []);
        this.memoryDB.store('onboarding', 'commands', this.onboardingState.commands || {});
        this.memoryDB.store('onboarding', 'importantFiles', this.onboardingState.importantFiles || []);
      } catch (_) { /* MemoryDB write is best-effort */ }
    }
  }
  ...
  saveAgent(agent) {
    ...
    // ── MemoryDB dual-write: persist agent state ─────────────────────────
    if (this.memoryDB) {
      try {
        this.memoryDB.storeAgentMemory(agent.agentType || 'unknown', `agent:${agent.id}`, {
          name: agent.name,
          status: agent.status,
          taskId: agent.taskId,
          teamId: agent.teamId || '',
  ...
  appendEvent(type, payload = {}, level = 'info') {
    ...
    // ── MemoryDB dual-write: mirror event to structured store ─────────────
    if (this.memoryDB) {
      try {
        this.memoryDB.appendEvent(type, { ...payload, level }, 'runtimeFeatures');
      } catch (_) { /* MemoryDB write is best-effort */ }
    }
    return event;
  }
  ...
  recordUsage(input = {}) {
    ...
    this.saveCosts();
    // ── MemoryDB dual-write: record usage metric ─────────────────────────
    if (this.memoryDB) {
      try {
        this.memoryDB.recordMetric('llm_usage', {
          calls: 1,
          promptTokens,
          completionTokens,
          embeddingTokens,
          model: input.model || ''
        }, { provider: this.providerId, taskId: input.taskId || '', agentId: input.agentId || '' });
      } catch (_) { /* MemoryDB write is best-effort */ }
    }
  }
  ...
  async searchWeb(query, limit = 5) {
    ...
    const order = [useProvider, ...providers.filter(p => p !== useProvider)];
    const lastError = [];
    ...
    for (const p of order) {
      try {
        switch (p) {
          ...
          case 'bing':
            if (!apiKey) {
              lastError.push('Bing selected but no API key configured');
              continue;
            }
            return await this._searchBing(query, limit, apiKey);
          case 'google':
            if (!apiKey) {
              lastError.push('Google selected but no API key configured');
              continue;
            }
            return await this._searchGoogle(query, limit, apiKey);
          default:
            lastError.push(`Unknown provider: ${p}`);
        }
      } catch (err) {
        lastError.push(`${p}: ${err.message}`);
        // Continue to next provider as fallback
      }
    }

    throw new Error(`All search providers failed: ${lastError.join('; ')}`);
  }
  ...
  async _searchGoogle(query, limit = 5, apiKey) {
    ...
    // Configure localai.web.searchApiKey as "API_KEY|CX".
    const count = Math.max(1, Math.min(10, Number(limit || 5)));
  ...
  async executeTool(name, input = {}, context = {}) {
    switch (name) {
      ...
      case 'wait_agent': {
        ...
        const timeoutMs = Math.max(1000, Math.min(30 * 60 * 1000, Number(input.timeoutMs || 120000)));
      ...
      case 'web_fetch':
        ...
        return this.fetchWebPage(String(input.url), input.maxChars != null ? Number(input.maxChars) : undefined);
